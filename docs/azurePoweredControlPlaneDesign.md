...
With complexity of distributed systems increasing, there has been an increasing need in having a Control Plane for these distributed systems for inventory control, capacity management, monitoring, and incident mitigation. While this requirement was first raised with horizontally scaled stamped applications, clients have come to realization of the benefits of such control plane and are looking to incorporate it to their software development.

Implementing a control plane is not an easy task and often introduces a lot overhead because of the complexity - there are security access requirements, policy enforcements, GUI, CLI ...etc. Not to mention it usually means a new set of tools for the users to get used to. Because of existing technology choices, increased adoption of OSS tools, multi-tenant and multi-platforms support, clients have to implement one off solution to address the gap from ground up. Over the past couple of years, more than 50% observed ISE control plane centric engagements were withdrawn. Some were because Microsoft consider it infeasible. Some were because the client lacked the capacity for execution.

## Azure Powered Control Plane Starter Kit

The starter kit uses Azure Managed Application for security, policy enforcement, inventory control, and capacity management. All components deployed in the managed application export metrics to log analytics workspace. Dashboards set up in Azure managed Grafana queries Azure Resource Graph to get resource deployment details and use SLO based health modeling to create a top level view of system health on a single pane of glass as well as providing the ability to drill down to individual components. Azure Monitor alerts or Azure Managed Grafana alerts can be set up to trigger alerts and incident mitigation.

The goal of the control plane is to set up a system that's efficient on resource allocation, responds to incident quickly, and self heals for high availability.
...
We use [Azure Managed Grafana](https://learn.microsoft.com/en-us/azure/managed-grafana/overview) to create dashboards using [KQL queries](https://learn.microsoft.com/en-us/azure/data-explorer/kusto/query/) using Azure Resource Graph and Azure Monitor data source exported to log analytics workspace. Because Log analytics workspace and Azure Resource Graph both work with multiple tenants and multiple subscriptions, it would all a top level dashboard on single pane of glass for simplicity and directness.
